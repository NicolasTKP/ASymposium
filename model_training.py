...
validation_data = scaled_data[train_end:]
...
X_train, y_train = create_sequences(train_data, n_steps=365)
X_validation, y_validation = create_sequences(validation_data, n_steps=365)
...
def training():
    ...
    model = Sequential([
        Input(shape=(365, 20)),
        LSTM(128, return_sequences=True),     # Return full sequence for next LSTM
        Dropout(0.2),
        LSTM(64, return_sequences=False),     # Return final output only
        Dropout(0.2),
        Dense(20)    
    ])
    ...
    dump(model, 'Model/wl_model.keras')
    dump(history, 'Model/model_history.joblib')
...
def evaluation():
    ...
    print("MSE:", mean_squared_error(y_validation, validation_pred))
    print("R² Score:", r2_score(y_validation, validation_pred))
